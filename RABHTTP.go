...
func NewProxyClientPool(opts CustomClientOptions) *ProxyClientPool {
...
func (p *ProxyClientPool) GetClientForProxy(proxy string) (*http.Client, error) {
	p.mu.Lock()
	defer p.mu.Unlock()

	if client, exists := p.clients[proxy]; exists {
		return client, nil
	}

	opts := p.options
	opts.UseProxy = true
	opts.Proxies = []string{proxy} // <-- CORREGIDO: convertir string en slice

	client, err := newInternalClient(opts)
	if err != nil {
		return nil, err
	}
...
func newInternalClient(options CustomClientOptions) (*http.Client, error) {
	dialer := &net.Dialer{Timeout: options.ConnectTimeout, KeepAlive: options.KeepAliveDuration, DualStack: true, Deadline: time.Now().Add(options.TotalTimeout)}
	...
	tlsConfig := &tls.Config{
		...
		Renegotiation:      options.Renegotiation,
		VerifyConnection: func(cs tls.ConnectionState) error {
			if !cs.HandshakeComplete || !cs.PeerCertificates[0].IsCA {
				return fmt.Errorf("TLS verification failed: incomplete handshake or missing CA cert")
			}
			return nil
		},
	}
...
func New(options CustomClientOptions) (*CustomHTTPClient, error) {
	...
	c := &CustomHTTPClient{
		...
		opts:             options,
		proxyPool:        NewProxyClientPool(options),
	}
...
func (c *CustomHTTPClient) doRequest(ctx context.Context, method, urlStr string, body io.Reader, headers http.Header, noRedirect bool) (*ProcessedHttpResponse, error) {
	defer func() {
		if r := recover(); r != nil {
			...
			err := &PanicError{
				Value:   r,
				Stack:   stack,
				Message: "panic recovered in HTTP request",
			}
	...
	for attempt := 0; attempt <= c.opts.RetryCount; attempt++ {
		...
		client, err := c.proxyPool.GetClientForProxy(proxyFinal)
		if err != nil {
...
func (c *CustomHTTPClient) Do(ctx context.Context, method, urlStr string, body io.Reader, headers http.Header) (*ProcessedHttpResponse, error) {
	return c.doRequest(ctx, method, urlStr, body, headers, false)
}
...
func (c *CustomHTTPClient) DoNoRedirect(ctx context.Context, method, urlStr string, body io.Reader, headers http.Header) (*ProcessedHttpResponse, error) {
	return c.doRequest(ctx, method, urlStr, body, headers, true)
}
