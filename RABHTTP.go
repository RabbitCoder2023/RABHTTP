...
func (c *CustomHTTPClient) doRequest(ctx context.Context, client *http.Client, method, urlStr string, body io.Reader, headers http.Header) (*ProcessedHttpResponse, error) {
	...
	// 1) Pre-leer el body para poder reutilizarlo en cada retry
	var payload []byte
	if body != nil {
		var err error
		payload, err = io.ReadAll(body)
		if err != nil {
			return nil, fmt.Errorf("error leyendo body para retry: %w", err)
		}
	}
